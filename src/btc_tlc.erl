...
do_btc_infer(Env, F) ->
    FunQName = util:getFnQName(F),
    ?PRINT(FunQName),
    {Env_, Type} = btc_infer(Env, [], F),
    % Use type to add to Env
    Env_.
...
-spec btc_infer(hm:env(), [any()], erl_syntax:syntaxTree()) ->
    {hm:env(), hm:type()}.
btc_infer(Env, _Tvs, {integer, L, _}) ->
    Inferred = hm:bt(integer, L),
    {Env, Inferred};
btc_infer(Env, _Tvs, {string, L,_}) ->
    Inferred = hm:tcon("List", [hm:bt(char,L)],L),
    {Env, Inferred};
btc_infer(Env, _Tvs, {char, L,_}) ->
    Inferred = hm:bt(char,L),
    {Env, Inferred};
btc_infer(Env, _Tvs, {float,L,_}) ->
    Inferred = hm:bt(float, L),
    {Env, Inferred};
btc_infer(Env, _Tvs, {atom,L,X}) ->
    Inferred = case X of
        B when is_boolean(B) -> hm:bt(boolean, L);
        _                    -> hm:bt(atom, L)
        end,
    {Env, Inferred};
btc_infer(Env, Tvs, {var, L, X}) ->
    case env:is_bound(X,Env) of
        true    ->
            {T, _} = lookup(X, Env, L),
            {Env, T};
...
btc_infer(Env, Tvs, {clause, L, _, _, _}=Clause) ->
    ClausePatterns = clause_patterns(Clause),
    ClauseBody = clause_body(Clause),
    {Env_1, A} = btc_infer(Env, Tvs, hd(ClausePatterns)),
    {Env_2, B} = hm:freshTMeta(Env_1, Tvs, 0),
    % Env_3 = env:extend(Name, A, Env_2),
    {Env_3, Type} = btc_check(Env_2, Tvs, ClauseBody, B),
    {Env_3, hm:funt([A], B, 0)};
btc_infer(Env, Tvs, Node) ->
    case type(Node) of
        Fun when Fun =:= function; Fun =:= fun_expr ->
            Clauses = case Fun of
                function -> function_clauses(Node);
            ...
            ClausesCheckRes = lists:map(fun(C) -> btc_infer(Env, Tvs, C) end, Clauses),
            Result = lists:foldl(
                fun({_Env, Res, _T}, AccRes) ->
                    AccRes and Res
                end, true, ClausesCheckRes),
...
inferClauseBody(Env, Tvs, BodyExprs) ->
    ...
    %     end, {Env,[],[]}, lists:droplast(BodyExprs)),
    % SolvedEnv = localConstraintSolver(Env_, CsBody, PsBody),
    btc_infer(Env, Tvs, lists:last(BodyExprs)).
